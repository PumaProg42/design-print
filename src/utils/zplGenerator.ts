...
export const generateZPL = (
  ...
  objects.forEach((obj: FabricObject) => {
    ...
    if (obj.type === "i-text") {
      ...
      // CODE object - use local ZPL generation (no API)
      ...
      const barcodeType: 'QR' | 'EAN_8' | 'EAN_13' | 'CODE_128' = 
        codeType === "qrcode" ? "QR" :
        codeType === "ean8" ? "EAN_8" :
        codeType === "ean13" ? "EAN_13" :
        codeType === "code128" ? "CODE_128" : "CODE_128";
      ...
      const element: BarcodeElementData = {
        type: barcodeType,
        value: codeData,
        x: cx,
        y: cy,
        ...
        qrMagnification: (obj as any).qrMagnification || estimateQrMagnification(widthScaled, codeData)
      };

      zpl += buildBarcodeZpl(element);
